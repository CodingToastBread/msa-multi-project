...
- **order-service** — orders keyed by `userId`. Writes go to Kafka (`orders` topic → JDBC Sink Connector → MariaDB `orders`, plus `example-catalog-topic` → catalog-service stock); reads use JPA against MariaDB (`DB_URL`). The old direct JPA insert and H2 config are kept commented out on purpose.
...
- **Persistence**: user-service and catalog-service use **in-memory H2** (`jdbc:h2:mem:testdb`), so data is wiped on restart (H2 console at `/h2-console`). order-service uses **MariaDB** (`docker compose up -d mariadb`), and new orders appear there only after the Sink Connector consumes them.
